...
description = "LaTeX 常用命令速查表：涵盖自定义字体、脚注设置、希腊字母、微积分公式、线性代数矩阵及各类符号表，助你写作效率翻倍。"
tags = ["LaTeX入门"]
dropCap = false
...
**Q: 矩阵或者表格太挤了怎么办？**
> **A**: 可以在换行符 `\\` 后指定额外的间距，例如 `\\[1ex]` 或 `\\[6pt]`。也可以在导言区设置 `\renewcommand{\arraystretch}{1.5}` 来全局增加行高。

**Q: 双引号怎么打？**
> **A**: 左引号用两个反引号 (键盘左上角)，右引号用两个单引号 `'`。即：`` ``Quote'' `` $\to$ “Quote”。

**Q: 如何让公式编号显示为 (1.1) 而不是 (1)？**
> **A**: 在导言区加入 `\numberwithin{equation}{section}`，这样公式编号就会跟随章节，如第一章的第一个公式为 (1.1)。

**Q: `align` 和 `equation` 环境有什么区别？**
> **A**: `equation` 用于单行公式，`align` 用于多行公式对齐。`align` 功能更强大，也支持单行，因此很多人习惯统用 `align`。注意：`align` 默认每行都编号，不想编号请用 `align*` 或 `\notag`。
...
| **强调** | `\emph{...}` | *text* | 动态切换斜体/正体 |
| **换行** | `\\` 或 `\newline` | | |
| **新页** | `\newpage` | | |
| **空格** | `\quad` | | 1em 宽度 |
| **大空格** | `\qquad` | | 2em 宽度 |
| **水平间距** | `\hspace{1cm}` | | 自定义长度 |
| **垂直间距** | `\vspace{1cm}` | | 自定义高度 |
| **居中** | `\begin{center}...\end{center}` | | 环境 |
| **左对齐** | `\begin{flushleft}...\end{flushleft}` | | 环境 |