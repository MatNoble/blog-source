...
series = ["latex"]
toc = true
description = "全面解析 LaTeX 中长公式折行、公式组对齐、分段函数 (cases) 以及 empheq 环境的高级用法，附带详细代码示例。" 
+++
...
| 环境名称 | 主要用途 | 对齐支持 | 默认编号 | 备注 |
| :--- | :--- | :---: | :---: | :--- |
| `equation` | 单行公式 | × | √ | 最基础，不可换行 |
| `multline` | 超长公式折行 | × | √ | 首行左对齐，末行右对齐 |
| `align` | 多行公式对齐 | √ (`&`) | √ | **最推荐**，功能最全 |
| `gather` | 多行公式堆叠 | × | √ | 仅居中堆叠，不对齐 |
| `cases` | 分段函数 | √ (`&`) | √ (整体) | 需配合 `numcases` 实现逐行编号 |
...
- `cases` 环境[^5]
  ...
  [^4]<img src="https://imgkr.cn-bj.ufileos.com/5ee5ecdf-8f8e-4606-8488-30e43ab4b871.png" alt="LaTeX 分段函数编号示例" width="90%"/>
